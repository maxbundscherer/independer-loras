...
ShortMessageAndTuple lora_short_message_parse(String msg, String myId) {
  ...
  for (int i = 0; i < msg_len; i++) {
    ...
    if (current == ":") {
      ...
      if (currentCountsDelimiter == 0) p_from += current;
      if (currentCountsDelimiter == 1) p_to += current;
      if (currentCountsDelimiter == 2) p_type += current;
      if (currentCountsDelimiter == 3) p_value += current;
    }
  }
