...
void gui_logo_static(String version_string, String my_id, String gateway_id, boolean isActor) {
  ...
  if (isActor) Heltec.display -> drawString(8, 35, version_string + " Actor\n" + my_id + " -> " + gateway_id);
  else Heltec.display -> drawString(8, 35, version_string + " Gateway\n" + gateway_id);
  ...
  while (!hasSelected) {
    ...
    while (Wire.available()) {
      ...
      if (c != 0) {
        ...
        if (curSelected < 0) curSelected = count_items;
        if (curSelected > count_items) curSelected = 0;

        if (c == 0xD) hasSelected = true; //Press Enter
  ...
  while (!hasSelected) {
    ...
    while (Wire.available()) {
      ...
      if (c != 0) {
        ...
        } else if (c == 0xD) hasSelected = true; //Press Enter
        else {
          current += c;
          current_length += 1;
        }
